...
const SKIP_EXTS: &[&str] = &[
    "lock", "sum", "png", "jpg", "jpeg", "gif", "svg", "ico", "woff", "woff2", "ttf", "eot", "pdf",
    "zip", "gz", "tar", "mp4", "mp3", "webm", "wav", "db", "sqlite", "bin", "exe",
];
...
impl Embedder {
    pub fn init(model_name: &str) -> Result<Self> {
        ...
        let embedding_model = EmbeddingModel::from_str(model_name)
            .map_err(|e| anyhow::anyhow!("Unknown embedding model '{}': {}", model_name, e))?;
        ...
        std::thread::spawn(move || {
            ...
            println!("  (Downloads model on first run to ~/.cache/huggingface)");
...
fn extract_file_summary(source: &str, rel_path: &str) -> String {
    ...
    let comment_summary = comment_lines
        .into_iter()
...
fn extract_signature(content: &str) -> Option<String> {
    ...
    // Strip trailing { or :
    let sig = first.trim_end_matches('{').trim_end_matches(':').trim_end();
    Some(sig.to_string())
}
...
pub async fn index_repo_into(
    ...
    for (i, file_path) in files.iter().enumerate() {
        ...
        if let Err(e) = index_file(
            db, embedder, vectors, repo_id, &repo.name, repo_path, file_path,
        )
        .await
        {
    ...
    tokio::spawn(async move {
        ...
        while let Some(event_result) = rx.recv().await {
            ...
            for path in &event.paths {
                if should_skip_file(path) || path.is_dir() {
                    continue;
                }

                let Some(repo) = repos.iter().find(|r| path.starts_with(&r.path)) else {
                    continue;
                };
                let repo_path = Path::new(&repo.path);
                let Ok(Some(repo_id)) = db.repo_id(&repo.name) else {
                    continue;
                };

                info!("  ↻ re-indexing: {}", path.display());
                let _ = index_file(
                    &db, &embedder, &vectors, repo_id, &repo.name, repo_path, path,
                )
                .await;
            }
        }
